...
class ImportDataFromDirectory(bpy.types.Operator):
    bl_idname = "sapling.importdatadir"
    bl_label = "Import Preset From Directory"
...
def importDataFromDir(filename):
    # Make sure the operator knows about the global variables
    print("import data from dir, sapling addon")
    global settings, useSet
    # Read the preset data into the global settings
    settings = {}
    try:
...
class AddTreeFromDirectory(bpy.types.Operator):
    bl_idname = "curve.tree_add_dir"
    bl_label = "Sapling: Add Tree From File"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        global settings, useSet
        start_time = time.time()
        print("Creating ", " tree...", sep = self.filename)
        # If we need to set the properties from a preset then do it here
        bpy.ops.sapling.importdatadir(filename = self.filename)
        if len(settings) != 0:
            for a, b in settings.items():
                setattr(self, a, b)
            utils.addTree(self)
        # cProfile.runctx("addTree(self)", globals(), locals())
            ...
            # print(self.filename, " tree not found")
            raise Exception("Tree not found in path {}".format(self.filename))
        return {"FINISHED"}
